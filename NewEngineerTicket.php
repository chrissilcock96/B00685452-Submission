...
$query0 = "SELECT *, MAX(created_at) FROM ticket GROUP BY ticketId ORDER BY created_at DESC LIMIT 1;";
        ...
        <div class="form-group">
            ...
            <div id="formContainer">
                <div id="form1">
                    ...
                    <form action="NewEngineerTicket.php" method="get" display: inline-block>
                                ...
                                <section class="container">
                                    ...
                                    <div class="left-half">
                                        <article>
                                            ...
                                            Description of Issue (Max 255 Characters)*<textarea class="form-control" NAME="description" rows="9" textarea maxlength="255" required style="width: 350px"><?php echo $row['description']?></textarea><br><Br>
                                            User Visible Note? <select name = "noteType"  style="width: 350px" class="form-control"/>  <option value="F">F</option>
                                            <option value="T">T</option></select><br>
                                            Add Notes<textarea name="notes" class="form-control" rows="9" textarea maxlength="255"  style="width: 350px"></textarea><br>
                                            <input type="submit" class="btn btn-primary" name="sbmtTicket" value="Save Ticket" />
                                        </article>
                                    </div>

                                    <div class="right-half">
                                        <article>
                                            Ticket Type <select name = "type"  style="width: 350px" class="form-control"/><option value="<?php echo $row['issueType'] ?>"selected hidden><?php echo $row['issueType'] ?></option><?PHP while ($rows = mysqli_fetch_array($result2)):;?> <option value="<?php echo $rows[1];?>"> <?php echo $rows[1];?></option> <?php endwhile;?></select><br>
                                            Sub-Type <select name = "subType"  style="width: 350px" class="form-control"/><option value="<?php echo $row['issueSubType'] ?>"selected hidden ><?php echo $row['issueSubType'] ?></option><?PHP while ($rows = mysqli_fetch_array($result4)):;?> <option value="<?php echo $rows[2];?>"> <?php echo $rows[2];?></option> <?php endwhile;?></select><br>
                                            Assigned Technician <select name = "assignedTech"  style="width: 350px" class="form-control"/><option value="<?php echo $row['assignedTechnician'] ?>"selected hidden ><?php echo $row['assignedTechnician'] ?></option><?PHP while ($rows = mysqli_fetch_array($result5)):;?> <option value="<?php echo $rows[2];?>"> <?php echo $rows[2];?></option> <?php endwhile;?></select><br>
                                            Date logged<input type="text" class="form-control" name="dateLogged" required style="width: 350px" readonly value="<?php echo $row['created_at'] ?>"/><br>
                                            Expected Completion Date<input type="text" class="form-control" name="compDate" required style="width: 350px" readonly value="<?php echo $date2?>"/><br>
                                            Last Edited<input type="text" class="form-control" name="editedDate" required style="width: 350px" readonly value="<?php echo $row['lastEditedTime'] ?>"/><br>
                                            Previous Notes:
                                            <textarea class="form-control  name = description" rows="9" textarea maxlength="255" readonly style="width: 76%"><?PHP while ($rows = mysqli_fetch_array($result2000)):;?><?php echo "CREATED ON: "," $rows[2]","\n\n"; echo "$rows[4] \n\n__________________________________________________________________\n\n"; endwhile;?></textarea>
...
// Check connection
    ...
    $sql = "update ticket set username='$requester',subject='$subject',description='$description',ticketStatus='$status',assignedTechnician='$assignedTech',ticketType='$priority',issueType='$type',issueSubtype='$subType',lastEditedTime='$date',sla ='$date2' where ticketId ='$ticket'";
